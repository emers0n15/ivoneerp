...
<html>
    <head>
      <meta charset="utf-8">
      <title>RI - Requisicao Interna</title>
                        ...
                        <tbody>
                            ...
                            $sql = "
                                SELECT 
                                    ...
                                    s.quantidade,
                                    s.prazo,
                                    s.quantidade_inicial  -- assumindo que você tem essa coluna para calcular o percentual
                                FROM produto as p
                                JOIN stock as s ON s.produto_id = p.idproduto
                                WHERE s.quantidade > 0 
                                AND s.prazo >= CURDATE()  -- Pega apenas produtos com prazo de validade não expirado
                                AND s.prazo = (
                                    SELECT MIN(s2.prazo)
                                    FROM stock as s2
                                    WHERE s2.produto_id = p.idproduto 
                                    AND s2.quantidade > 0
                                    AND s2.prazo >= CURDATE()  -- Garante que a validade do lote não está expirada
                                )
                            ";
                            $rs = mysqli_query($db, $sql);
    ...
    <script>
        $(function(){
            ...
            $("#qrcode").on('change', function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "ris/addarttempbarcode.php", // Especifique o arquivo PHP desejado
            ...
            function handleClick() {
                ...
                $.ajax({
                    type: "POST",
                    url: "ris/addarttemp.php", // Especifique o arquivo PHP desejado
                ...
                if (confirm("Tem certeza de que deseja efetuar o pagamento?")) {
                    ...
                    if(sector == ""){
                       ...
                       //alert(cliente+":cliente modo:"+modo+" valor:"+valor+" mesa:"+mesa);
                        $.ajax({
                            type: "POST",
                            url: "ris/criarrqinterna.php", // Especifique o arquivo PHP desejado
                            data: {
                              sector: sector,
                              utente: $(".utente").val(),
                              motivo: $(".motivo").val()
                            ...
                            success: function(data){
                                if(data == 2000000000000000000000000){
                                  ...
                                  window.open("ri_pdf.php?id_ri="+data+"");
                                  location.reload();
                                }
                            },
                            error: function(){
            ...
            $("#cancelar").click(function() {
              if (confirm("Tem certeza de que deseja remover estes items?")){
                $.ajax({
                        url: 'ris/artremoveall.php', // Seu arquivo PHP para remover o item
                        method: 'POST',
                        success: function(response) {
                            popprodtable();
                            qrcodefocus();
            ...
            function btnremover(idProduto){
                ...
                if (confirm("Tem certeza de que deseja remover este item?")) {
                    ...
                    $.ajax({
                        url: 'ris/artremove.php', // Seu arquivo PHP para remover o item
                        method: 'POST',
