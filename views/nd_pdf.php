...
function fetch_data($d, $id_n) {
    ...
    $sql = "SELECT id, (SELECT nomeproduto FROM produto as p WHERE p.idproduto = f.artigo) as artigo, qtd, preco, total,iva FROM nd_artigos as f WHERE id_nd = ? ";
...
$pdf->SetTitle('Nota de Débito #' . $id_nd); // Defina o título do PDF
...
$sql1 = "SELECT * FROM clientes WHERE id = (SELECT cliente FROM nota_debito WHERE id = '$id_nd')";
$rs1 = mysqli_query($db, $sql1);
$dados1 = mysqli_fetch_array($rs1);

// Dados da Factura
$sql2 = "SELECT *, (SELECT serie FROM factura WHERE factura.id = nota_debito.id_factura) as sr, (SELECT n_doc FROM factura WHERE factura.id = nota_debito.id_factura) as nd FROM nota_debito WHERE id = '$id_nd'";
...
$pdf->Cell(0, 10, 'Nota de Débito #'.$dados2['serie'].'/'.$id_nd, 0, 1);
...
$pdf->SetX(160);
$pdf->Cell(0, 7, 'Referência a Factura #', 'B', 0);
$pdf->SetFont('helvetica', '', 8);
$pdf->Ln(7);
$pdf->Cell(0, 7, $dados2['apolice'], 0, 0);
$pdf->SetX(60);
$pdf->Cell(0, 7, $dados2['codigo1'], 0, 0);
$pdf->SetX(120);
$pdf->Cell(0, 7, $dados2['codigo2'], 0, 0);
$pdf->SetX(160);
$pdf->Cell(0, 7, $dados2['sr'].'/'.$dados2['nd'], 0, 0);
$pdf->Ln(7);
...
// Output do PDF
$pdf->Output('Nota_de_debito.pdf', 'I');
