...
function fetch_data($d, $id_r) {
    ...
    $sql = "SELECT id, (SELECT nomeproduto FROM produto as p WHERE p.idproduto = f.artigo) as artigo, qtd FROM ri_artigos as f WHERE ri = ? ";
...
$sql1 = "SELECT * FROM sector WHERE id = (SELECT sector FROM requisicao_interna WHERE id = '$id_ri')";
$rs1 = mysqli_query($db, $sql1);
$dados1 = mysqli_fetch_array($rs1);

// Dados da Factura
$sql2 = "SELECT * FROM requisicao_interna WHERE id = '$id_ri'";
$rs2 = mysqli_query($db, $sql2);
$dados2 = mysqli_fetch_array($rs2);

// Criar documento com cabeçalho padronizado e logotipo
$titulo = 'REQUISIÇÃO INTERNA #'.$dados2['serie'].'/'.$id_ri;
$pdf = new ThemedTCPDF('P', 'mm', 'A4', true, 'UTF-8', false, $empresa, $titulo);
$pdf->AddPage();

// Metadados do documento
$pdf->SetCreator('IvoneERP');
$pdf->SetTitle('Requisição Interna');

// Bloco de informações
$pdf->SetFont('helvetica', '', 9);
$pdf->Cell(0, 6, 'Requisitante: ' . ($dados1['nome'] ?? ''), 0, 1);
$pdf->Cell(0, 6, 'Responsável: ' . ($dados1['responsavel'] ?? ''), 0, 1);
$pdf->Cell(0, 6, 'A Requisitar: ' . ($dados2['solicitante'] ?? ''), 0, 1);
$pdf->Cell(0, 6, 'Data de Emissão: ' . ($dados2['data'] ?? ''), 0, 1);
$pdf->Cell(0, 6, 'Motivo de Emissão: ' . ($dados2['motivo'] ?? ''), 0, 1);
$pdf->Ln(4);
...
$stmt = mysqli_prepare($db, "SELECT id, (SELECT nomeproduto FROM produto as p WHERE p.idproduto = f.artigo) as artigo, qtd FROM ri_artigos as f WHERE ri = ? ");
mysqli_stmt_bind_param($stmt, "s", $id_ri);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
$rowIndex = 0;
