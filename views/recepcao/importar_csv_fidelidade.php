<?php
/**
 * Script de Importação Específico para CSV da Seguradora Fidelidade
 * Importa diretamente do ficheiro CSV especificado no desktop do utilizador
 *
 * Formato esperado do CSV:
 * Ordem;Sector;Nome do Procedimento;Preço
 */
...
// Verificar se empresa/seguradora Fidelidade existe
$sql_empresa = "SELECT id, nome FROM empresas_seguros 
                WHERE nome LIKE '%Fidelidade%' 
                ORDER BY id DESC LIMIT 1";
$rs_empresa  = mysqli_query($db, $sql_empresa);
$empresa     = mysqli_fetch_array($rs_empresa);

if(!$empresa){
    die('Empresa/Seguradora "Fidelidade" não encontrada na base de dados. '
...
while(($linha = fgetcsv($handle, 5000, ';')) !== FALSE){
    ...
    // Ignorar linhas vazias ou com poucas colunas
    if(empty(array_filter($linha)) || count($linha) < 4){
        continue;
    }

    // Estrutura: Ordem;Sector;Nome do Procedimento;Preço
    $codigo    = trim($linha[0]);
    $sector    = trim($linha[1]);
    $nome      = trim($linha[2]);
    ...
    // Converter preço (formato PT: 9.557,31 → 9557.31)
    $preco = converterPrecoFidelidade($preco_str);
    ...
    garantirCategoriaFidelidade($db, $sector, $userID);
    ...
    // Configurar preço específico para a seguradora Fidelidade
    if(configurarPrecoEmpresaFidelidade($db, $empresa_id, $servico_id, $preco, $userID)){
        $estatisticas['precos_configurados']++;
    } else {
        $erros[] = "Linha $linha_num: Erro ao configurar preço para Fidelidade.";
        $estatisticas['erros']++;
    }
}
...
function converterPrecoFidelidade($preco_str){
...
function garantirCategoriaFidelidade($db, $nome_categoria, $userID){
...
function configurarPrecoEmpresaFidelidade($db, $empresa_id, $servico_id, $preco, $userID){
    $tabela_precos_id = obterOuCriarTabelaPrecosFidelidade($db, $empresa_id, $userID);
...
function obterOuCriarTabelaPrecosFidelidade($db, $empresa_id, $userID){
    // Verificar se já existe tabela ativa
...
<head>
    ...
    <title>Resultado da Importação - Fidelidade</title>
...
<body>
    <div class="container mt-4">
        <h2>📊 Resultado da Importação - Seguradora Fidelidade</h2>
        ...
        <div class="mt-4">
            ...
            <a href="tabela_precos.php?id=<?php echo $empresa_id; ?>" class="btn btn-success">Ver Preços da Fidelidade</a>
