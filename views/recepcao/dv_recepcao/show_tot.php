...
if($empresa_id && $empresa_id > 0) {
    $sql = "SELECT SUM(total) as t FROM dv_servicos_temp as st WHERE user = ? AND empresa_id = ?";
    $stmt = mysqli_prepare($db, $sql);
    mysqli_stmt_bind_param($stmt, "ii", $userID, $empresa_id);
    mysqli_stmt_execute($stmt);
    $rs = mysqli_stmt_get_result($stmt);
} else {
    $sql = "SELECT SUM(total) as t FROM dv_servicos_temp as st WHERE user = ? AND (empresa_id IS NULL OR empresa_id = 0)";
    $stmt = mysqli_prepare($db, $sql);
