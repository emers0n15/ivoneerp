...
$sql_check = "SELECT qtd FROM vds_servicos_temp WHERE id = ?";
...
if($dados_check && $dados_check['qtd'] > 1) {
    $sql = "UPDATE vds_servicos_temp SET qtd = qtd - 1, total = (qtd - 1) * preco WHERE id = ?";
    $stmt = mysqli_prepare($db, $sql);
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
} else {
    $sql = "DELETE FROM vds_servicos_temp WHERE id = ?";
    $stmt = mysqli_prepare($db, $sql);
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
}
