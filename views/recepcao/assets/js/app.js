...
$(document).ready(function($) {
	...
	$(document).on('click', '#mobile_btn', function() {
		...
		// Adicionar classe active ao sidebar em mobile
	...
	// Fechar sidebar ao clicar em link do menu (mobile)
	...
	// Small Sidebar - APENAS por clique, SEM hover
	if(screen.width >= 992) {
		$(document).on('click', '#toggle_btn', function() {
			if($('body').hasClass('mini-sidebar')) {
				$('body').removeClass('mini-sidebar');
				$('body').removeClass('expand-menu'); // Garantir remoção
				$('.subdrop + ul').slideDown();
			} else {
				$('body').addClass('mini-sidebar');
				$('body').removeClass('expand-menu'); // Garantir que não expanda
				$('.subdrop + ul').slideUp();
			}
			return false;
		});
		
		// REMOVER COMPLETAMENTE qualquer comportamento de hover/mouseover
		// Não adicionar expand-menu por hover
		$(document).off('mouseover', '.mini-sidebar .sidebar');
		$(document).off('mouseenter', '.mini-sidebar .sidebar');
	}
});
