<?php 
    ...
    $sql_limpar = "DELETE FROM fa_servicos_temp WHERE user = ?";
...
<html>
    <head>
      <meta charset="utf-8">
      <title>FA - Factura (Recepção)</title>
              ...
              <div class="modal fade" id="exampleModal3" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            ...
                            <select class="form-select metodo" aria-label="Default select example" id="select" style="width: 100%;padding: 5px 0;">
                              ...
                              <option value="fatura_empresa">Fatura para Empresa</option>
                            </select>
                          </div>
                        </div><br>
                        <div class="row">
                        ...
                        <div class="row">
                          ...
                          <div class="col-sm-4">
                            <label class="form-label">Codigo de Autorizacao 3</label>
                            <input type="text" class="form-control codigo3" id="codigo3" placeholder="Opcional">
                          </div>
                        </div>
    ...
    <script>
        ...
        $(function(){
            ...
            function popprodtable(){
                var url = 'fas_recepcao/popprodtable.php';
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        // Anexar eventos usando delegação
            ...
            function decreaseQuantity(idServico) {
                $.ajax({
                    url: 'fas_recepcao/qtd_decrease.php',
                    type: 'POST',
                    data: {
                            id: idServico
                    },
            ...
            function increaseQuantity(idServico) {
                $.ajax({
                    url: 'fas_recepcao/qtd_increase.php',
                    type: 'POST',
                    data: {
                            id: idServico
                    },
            ...
            function updateQuantity(idServico, novoValor) {
                $.ajax({
                    url: 'fas_recepcao/qtd_update.php',
                    type: 'POST',
                    data: {
                            id: idServico,
                            novoValor: novoValor
            ...
            function showtot() {
                var url = 'fas_recepcao/show_tot.php';
                if(empresaSelecionada) {
                    url += '?empresa=' + empresaSelecionada;
                }
                $.ajax({
            ...
            function handleClick() {
                ...
                $.ajax({
                    type: "POST",
                    url: "fas_recepcao/addservtemp.php",
                    data: { 
                        idServico: idServico,
            ...
            function criarpedido() {
                if (confirm("Tem certeza de que deseja efetuar o pagamento?")) {
                    ...
                    if(!paciente || paciente == "" || paciente == null){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "fas_recepcao/criarpedido.php",
                            data: {
                              paciente: paciente,
                              empresa_id: empresaSelecionada,
                              prazo: $(".prazo").val(),
                            ...
                            success: function(data){
                                if (data == 1) {
                                  ...
                                  alert('Erro ao criar fatura. Por favor, verifique se as tabelas factura_recepcao e fa_servicos_fact_recepcao foram criadas e tente novamente.');
                                }else if(data == 3){
                                  alert('Não é possível criar uma fatura para um ano diferente do atual!\n\nPor favor, atualize o ano fiscal no dashboard para o ano atual (' + new Date().getFullYear() + ').');
                                  window.location.href = 'dashboard.php';
                                }else if(data == 4){
                                  alert('Erro: As tabelas necessárias não foram criadas.\n\nPor favor, execute o script SQL create_factura_recepcao_tables.sql para criar as tabelas factura_recepcao e fa_servicos_fact_recepcao.');
                                }else{
                                  alert('Fatura criada com sucesso! ID: ' + data);
                                  location.reload();
                                }
                            },
                            error: function(){
            ...
            // Função para carregar pacientes baseado na empresa selecionada
            function carregarPacientes(empresaId) {
                var url = 'fas_recepcao/buscar_pacientes.php';
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Adicionar pacientes (data já é um array)
                        ...
                        // Garantir que nenhum paciente está selecionado por padrão
            ...
            $("#cancelar").click(function() {
              if (confirm("Tem certeza de que deseja remover estes items?")){
                $.ajax({
                        url: 'fas_recepcao/removeservall.php',
                        method: 'POST',
                        success: function(response) {
                            popprodtable();
                            showtot();
            ...
            function btnremover(idServico){
                if (confirm("Tem certeza de que deseja remover este item?")) {
                    $.ajax({
                        url: 'fas_recepcao/removeservtemp.php',
                        method: 'POST',
