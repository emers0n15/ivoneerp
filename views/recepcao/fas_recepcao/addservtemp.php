...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    // Buscar preço da empresa se houver
    ...
    // Serviço não estocável - apenas insira diretamente
    $qt = 1;
    $total = $preco;

    // Verificar se o serviço já está na tabela temporária (mesmo serviço, mesmo usuário, mesma empresa)
    if($empresa_id && $empresa_id > 0) {
        $sql = "SELECT * FROM fa_servicos_temp WHERE servico = ? AND user = ? AND empresa_id = ?";
        $stmt = mysqli_prepare($db, $sql);
        mysqli_stmt_bind_param($stmt, "iii", $idServico, $userID, $empresa_id);
    } else {
        $sql = "SELECT * FROM fa_servicos_temp WHERE servico = ? AND user = ? AND (empresa_id IS NULL OR empresa_id = 0)";
        $stmt = mysqli_prepare($db, $sql);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if($empresa_id && $empresa_id > 0) {
            $sql = "UPDATE fa_servicos_temp SET qtd = qtd + 1, total = (qtd + 1)*preco WHERE servico = ? AND user = ? AND empresa_id = ?";
            $stmt = mysqli_prepare($db, $sql);
            mysqli_stmt_bind_param($stmt, "iii", $idServico, $userID, $empresa_id);
        } else {
            $sql = "UPDATE fa_servicos_temp SET qtd = qtd + 1, total = (qtd + 1)*preco WHERE servico = ? AND user = ? AND (empresa_id IS NULL OR empresa_id = 0)";
            $stmt = mysqli_prepare($db, $sql);
            mysqli_stmt_bind_param($stmt, "ii", $idServico, $userID);
        }
        ...
        if($empresa_id && $empresa_id > 0) {
            $sql = "INSERT INTO fa_servicos_temp(servico, qtd, preco, total, user, empresa_id) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($db, $sql);
            mysqli_stmt_bind_param($stmt, "iiddii", $idServico, $qt, $preco, $total, $userID, $empresa_id);
        } else {
            $sql = "INSERT INTO fa_servicos_temp(servico, qtd, preco, total, user) VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($db, $sql);
    ...
    echo 10; // Serviço não encontrado
    exit;
}
?>
