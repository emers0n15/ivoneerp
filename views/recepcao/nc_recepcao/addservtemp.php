...
$check_table = "SHOW TABLES LIKE 'nc_servicos_temp'";
$table_exists = mysqli_query($db, $check_table);
if(!$table_exists || mysqli_num_rows($table_exists) == 0) {
    error_log("Erro: Tabela nc_servicos_temp não existe");
    echo 4;
    exit;
}
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    if($empresa_id && $empresa_id > 0) {
        $sql = "SELECT * FROM nc_servicos_temp WHERE servico = ? AND user = ? AND empresa_id = ?";
        $stmt = mysqli_prepare($db, $sql);
        mysqli_stmt_bind_param($stmt, "iii", $idServico, $userID, $empresa_id);
    } else {
        $sql = "SELECT * FROM nc_servicos_temp WHERE servico = ? AND user = ? AND (empresa_id IS NULL OR empresa_id = 0)";
        $stmt = mysqli_prepare($db, $sql);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        if($empresa_id && $empresa_id > 0) {
            $sql = "UPDATE nc_servicos_temp SET qtd = qtd + 1, total = (qtd + 1)*preco WHERE servico = ? AND user = ? AND empresa_id = ?";
            $stmt = mysqli_prepare($db, $sql);
            mysqli_stmt_bind_param($stmt, "iii", $idServico, $userID, $empresa_id);
        } else {
            $sql = "UPDATE nc_servicos_temp SET qtd = qtd + 1, total = (qtd + 1)*preco WHERE servico = ? AND user = ? AND (empresa_id IS NULL OR empresa_id = 0)";
            $stmt = mysqli_prepare($db, $sql);
            mysqli_stmt_bind_param($stmt, "ii", $idServico, $userID);
        }
        ...
        if($empresa_id && $empresa_id > 0) {
            $sql = "INSERT INTO nc_servicos_temp(servico, qtd, preco, total, user, empresa_id) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($db, $sql);
            mysqli_stmt_bind_param($stmt, "iiddii", $idServico, $qt, $preco, $total, $userID, $empresa_id);
        } else {
            $sql = "INSERT INTO nc_servicos_temp(servico, qtd, preco, total, user) VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($db, $sql);
