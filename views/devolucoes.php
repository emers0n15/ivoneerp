                      ...
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h3 class="modal-title fs-5" id="exampleModalLabel">Detalhes da Devolucao</h3>
                          ...
                          <div class="modal-body">
                            ...
                            <br>
                            <label class="form-label">Metodo de Devolucao</label><br>
                            <select class="form-select modo" aria-label="Default select example" id="select" style="width: 100%;padding: 5px 0;">
                              <option selected value="Numerario">Numerario</option>
                            ...
                            <textarea class="form-control motivo" id="motivo">
                          ...
                          <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary btn_criar">Efetuar Devolucao</button>
                          </div>
                        </div>
                      </div>
              </div>
            ...
            <div class="row">
                ...
                <div class="col-sm-8 segunda1 table-responsive" style="background:#fff;display: flex;flex-direction: row;overflow: auto;height:90vh;color: #333;justify-content: center;padding-top: 1%;">
                    <table id="example" style="height: 86%;width: 100%;">
                    	...
                    	<tbody>
                    		<?php
							    $sql = "SELECT *, (SELECT CONCAT(nome,' ',apelido) FROM clientes as c WHERE c.id = p.clientepedido) as nms FROM pedido as p WHERE devolucao = 0";
							    $rs = mysqli_query($db, $sql);
							    while ($dados = mysqli_fetch_array($rs)) {
							?>
							    <tr data-idpedido="<?php echo $dados['idpedido']; ?>">
							        <td><?php echo $dados['idpedido'] ?></td>
							        <td><?php echo "VD ".$dados['serie']."/".$dados['n_doc']; ?></td>
							        <td><?php echo $dados['nms'] ?></td>
							        <td><?php echo $dados['pagamentopedido'] ?></td>
                                    <td><?php echo $dados['iva'] ?></td>
                                    <td><?php echo ($dados['pagamentopedido']+ $dados['iva']); ?></td>
    ...
    <script>
        $(function(){
            ...
            $("#qrcode").on('change', function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "dvs/addarttempbarcode.php", // Especifique o arquivo PHP desejado
                ...
                $.ajax({
                    type: "POST",
                    url: "dvs/addtempinfobarcode.php", // Especifique o arquivo PHP desejado
                    data: { codbar: codbar }, // Passa o valor do input como um par0‰9metro
                    success: function(response){
                        var dados = JSON.parse(response);
                        $('.cliente').val(dados.clientepedido).trigger('change'); // Selecionar o cliente
                        $('.modo').val(dados.modo); // Selecionar o método de pagamento
                        console.log(dados);
                    }
                });
            });
            ...
            function handleClick() {
                var idpedido = this.getAttribute('data-idpedido'); // Obt¨¦m o valor do input dentro da div clicada
                $.ajax({
                    type: "POST",
                    url: "dvs/addarttemp.php", // Especifique o arquivo PHP desejado
                    data: { idpedido: idpedido }, // Passa o valor do input como um par0‰9metro
                    success: function(response){
                        if(response == 1){
                            alert("O caixa encontra-se fechado para este usuario! Abra o caixa na seccao caixa no menu lateral esquerdo.");
                        } else if(response == 3){
                            ...
                            alert("Ocorreu um erro ao adicionar os artigos! Contacte o administrador do sistema.");
                        } else if(response == 40){
                            alert("Nao foram enviados parametros para a devida requisicao.");
                        }
                        console.log(response);
                ...
                $.ajax({
                    type: "POST",
                    url: "dvs/addtempinfo.php", // Especifique o arquivo PHP desejado
                    data: { idpedido: idpedido }, // Passa o valor do input como um par0‰9metro
                    success: function(response){
                        var dados = JSON.parse(response);
                        $('.cliente').val(dados.clientepedido).trigger('change'); 
                        $('.idp').val(dados.idpedido);
                    }
                });
            ...
            function criardevolucao() {
                if (confirm("Tem certeza de que deseja efetuar o pagamento?")) {
                    ...
                    if(cliente == ""){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "dvs/criardevolucao.php", // Especifique o arquivo PHP desejado
                            data: { cliente: cliente, modo: modo, idp: idp, motivo: motivo}, // Passa o valor do input como um par0‰9metro
                            success: function(response){
                                if(response == 300000000000000000000){
                                    alert("O valor neste caixa e inferior ao valor a devolver.");
                                }else if(response == 450000000000000000000){
                                    ...
                                    window.open("dv_pdf.php?id_dv="+response+"");
								    location.reload();
                                }
                                console.log(response);
                            },
            ...
            $('#exampleModal3').on('shown.modal', function () {
              $('.js-example-basic-single1').select2({
                dropdownParent: $('#exampleModal3 .modal-body') // Defina o pai do dropdown como o corpo do modal
              });
            });
            ...
            $("#cancelar").click(function() {
            	if (confirm("Tem certeza de que deseja remover este item?")){
            		$.ajax({
                        url: 'dvs/artremoveall.php', // Seu arquivo PHP para remover o item
                        method: 'POST',
                        success: function(response) {
                            popprodtable();
                            showtot();
            ...
            function btnremover(idProduto){
                ...
                if (confirm("Tem certeza de que deseja remover este item?")) {
                    ...
                    $.ajax({
                        url: 'dvs/artremove.php', // Seu arquivo PHP para remover o item
                        method: 'POST',
