                ...
                if ($result) {
                    if(mysqli_num_rows($result) > 0) {
                        ...
                        while ($dados1 = mysqli_fetch_array($result)) {
                            ...
                            $iva = $dados1['iva'];
                            $sql = "INSERT INTO `artigos_devolvidos_temp`( `artigo`, `qtd`, `preco`, `total`, `iva`, `user`) VALUES(?,?,?,?,?,?)";
                            $stmt = mysqli_prepare($db, $sql);
                            mysqli_stmt_bind_param($stmt, "iidddi", $produtoentrega, $qtdentrega, $precoentrega, $totalentrega, $iva, $userID);
                            mysqli_stmt_execute($stmt);
