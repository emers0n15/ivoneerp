...
function fetch_data($d, $id_e) {
    $output = '';
    
    // Use uma consulta preparada para evitar injeção de SQL
    $sql = "SELECT id, (SELECT nomeproduto FROM produto as p WHERE p.idproduto = f.artigo) as artigo, qtd FROM es_artigos as f WHERE es = ? ";
    ...
    if ($stmt) {
        ...
        mysqli_stmt_bind_param($stmt, "s", $id_e);
...
$pdf->SetTitle('Entrada de Stock #' . $id_es); // Defina o título do PDF
...
$sql1 = "SELECT * FROM grupo_artigos WHERE id = (SELECT grupo FROM entrada_stock WHERE id = '$id_es')";
$rs1 = mysqli_query($db, $sql1);
$dados1 = mysqli_fetch_array($rs1);


// Dados da Empresa Faturada (exemplo)
$sql4 = "SELECT * FROM familia_artigos WHERE id = (SELECT familia FROM entrada_stock WHERE id = '$id_es')";
$rs4 = mysqli_query($db, $sql4);
$dados4 = mysqli_fetch_array($rs4);

// Dados da Factura
$sql2 = "SELECT * FROM entrada_stock WHERE id = '$id_es'";
...
$pdf->SetX(140);
$pdf->Cell(0, 7, 'Familia dos Artigos:', 0, 1);
$pdf->SetX(140);
$pdf->Cell(0, 7, $dados4['descricao'], 0, 1);
$pdf->SetX(140);
$pdf->Cell(0, 7, 'Grupo dos Artigos:', 0, 1);
$pdf->SetX(140);
$pdf->Cell(0, 7, $dados1['descricao'], 0, 1);



$pdf->SetFont('helvetica', 'B', 12);
$pdf->Cell(0, 10, 'Entrada de Stock #'.$dados2['serie'].'/'.$n_doc, 0, 1);
...
$pdf->SetX(50);
$pdf->Cell(0, 7, 'Lote', 'T', 0);
$pdf->SetX(90);
$pdf->Cell(0, 7, 'Validade do Lote', 'T', 0);
$pdf->SetFont('helvetica', '', 8);
$pdf->Ln(7);
...
// Output do PDF
$pdf->Output('Entrada_stock.pdf', 'I');
