...
if ($year == $serie) {
    ...
    $sql3 = "INSERT INTO saida_stock(n_doc, `serie`, `solicitante`, `motivo`, `user`) VALUES('$new_id','$serie','$utente', '$motivo', '$userID')";
$rs3 = mysqli_query($db, $sql3) or die(mysqli_error($db));
if ($rs3 > 0) {
    $sql5 = "SELECT MAX(id) as id FROM saida_stock WHERE user = '$userID'";
    $rs5 = mysqli_query($db, $sql5) or die(mysqli_error($db));
    if (mysqli_num_rows($rs5) > 0) {
        $dados5 = mysqli_fetch_array($rs5);
        $id_ss = $dados5['id'];

        $sql4 = "SELECT * FROM ss_artigos_temp WHERE user = '$userID'";
        $rs4 = mysqli_query($db, $sql4) or die(mysqli_error($db));
        while ($dados4 = mysqli_fetch_array($rs4)) {
            $artigo = $dados4['artigo'];
            $qtd = $dados4['qtd'];
            $sql6 = "INSERT INTO `ss_artigos`(`artigo`, `qtd`, `ss`, `user`) VALUES('$artigo', '$qtd', '$id_ss', '$userID')";
            $rs6 = mysqli_query($db, $sql6) or die(mysqli_error($db));

            $sql10 = "UPDATE `produto` SET `stock` = stock - '$qtd' WHERE `idproduto` = '$artigo'";
            $rs10 = mysqli_query($db, $sql10) or die(mysqli_error($db));

        }
    }
    $sql8 = "DELETE FROM ss_artigos_temp WHERE user = '$userID'";
