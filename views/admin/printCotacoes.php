...
$columns = [
    ['#', 12], ['Descrição', 35], ['Valor', 22], ['IVA', 18], ['Total', 25], ['Data de Emissão', 38], ['Prazo', 40]
];
tcpdf_table_header($pdf, $columns);

// Loop while para adicionar linhas de itens
$pdf->SetFont('helvetica', '', 8);
$sql3 = "SELECT DISTINCT(cliente) as cl FROM `cotacao` as e WHERE data BETWEEN '$data1' AND '$data2'";
...
while ($dados3 = mysqli_fetch_array($rs3)) {
	...
	$pdf->Ln(6);
	$pdf->SetFont('helvetica', '', 8);
	$sqlx = "SELECT * FROM cotacao as f WHERE cliente = '$cl' AND f.data BETWEEN '$data1' AND '$data2' AND serie = '$serie' ORDER BY f.data";
	...
	while ($dadosx = mysqli_fetch_array($rsx)) {
		...
		$pdf->Cell(35, 6, "CT#".$dadosx['serie']."/".$dadosx['n_doc'], 1, 0, 'L', $fill);
...
// Output do PDF
$pdf->Output('Relatorio_cotacoes.pdf', 'I');
