...
if (isset($_POST['btn'])) {
	...
	$sql = "INSERT INTO saida_caixa(`serie`, `valor`, `user`, `caixa`) VALUES('$serie', '$valor', '$userID', '$caixa')";
	$rs = mysqli_query($db, $sql) or die(mysqli_error($db));
	if($rs > 0){
		$sql = "UPDATE periodo SET fechoperiodo = fechoperiodo - '$valor' WHERE diaperiodo = 'Aberto' AND idperiodo = '$caixa'";
		$rs = mysqli_query($db, $sql) or die(mysqli_error($db));
		if ($rs > 0) {
			echo "<script>alert('Saída inserida com sucesso!!'); </script>";
			echo "<script>window.location='../saida_caixa.php'; </script>";
		}else{
		echo "<script>alert('Cliente não inserido.....Por favor tente novamente!'); </script>";
		echo "<script>window.location='../saida_caixa_conf.php'; </script>";
	}
	}
