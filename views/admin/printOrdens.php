...
$titulo = 'RELATÓRIO DE ORDEM DE COMPRA - ' . $data1 . ' / ' . $data2;
$pdf = new ThemedTCPDF('P', 'mm', 'A4', true, 'UTF-8', false, $dados, $titulo);
$pdf->SetCreator('IvoneERP');
$pdf->SetTitle('Relatório de Ordem de Compra');
$pdf->AddPage();
...
while ($dados3 = mysqli_fetch_array($rs3)) {
	...
	$pdf->Ln(6);
	$pdf->SetFont('helvetica', '', 8);
	$sqlx = "SELECT * FROM ordem_compra as f WHERE fornecedor = '$cl' AND f.data BETWEEN '$data1' AND '$data2' AND serie = '$serie' ORDER BY f.data";
	...
	while ($dadosx = mysqli_fetch_array($rsx)) {
		...
		$pdf->Cell(35, 6, "OC#".$dadosx['serie']."/".$dadosx['n_doc'], 1, 0, 'L', $fill);
...
// Output do PDF
$pdf->Output('Relatorio_ordem_compra.pdf', 'I');
