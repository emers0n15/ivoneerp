...
$columns = [
    ['#', 12], ['Descrição', 48], ['Valor', 25], ['Modo de Pagamento', 55], ['Data', 50]
];
...
$sql3 = "SELECT DISTINCT(cliente) as cl FROM `recibo` as e WHERE data BETWEEN '$data1' AND '$data2'";
...
while ($dados3 = mysqli_fetch_array($rs3)) {
	...
	$pdf->Ln(6);
	$pdf->SetFont('helvetica', '', 8);
	$sqlx = "SELECT * FROM recibo as f WHERE cliente = '$cl' AND f.data BETWEEN '$data1' AND '$data2' AND serie = '$serie' ORDER BY f.data";
	...
	while ($dadosx = mysqli_fetch_array($rsx)) {
		...
		$pdf->Cell(48, 6, "RC#".$dadosx['serie']."/".$dadosx['n_doc'], 1, 0, 'L', $fill);
		$pdf->Cell(25, 6, number_format($dadosx['valor'], 2), 1, 0, 'R', $fill);
		$pdf->Cell(55, 6, $dadosx['modo'], 1, 0, 'C', $fill);
		$pdf->Cell(50, 6, $dadosx['data'], 1, 1, 'C', $fill);
...
// Output do PDF
$pdf->Output('Relatorio_recibos.pdf', 'I');
