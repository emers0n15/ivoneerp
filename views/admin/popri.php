...
$countSql = "SELECT COUNT(*) AS total FROM requisicao_interna";
...
$filterSql = "
    SELECT COUNT(*) AS filtered FROM requisicao_interna AS d
    WHERE
        d.id LIKE ? OR
        d.motivo LIKE ? OR
        CONCAT('RI#', d.serie, '/', d.n_doc) LIKE ? OR
        (SELECT nome FROM sector WHERE sector.id = d.sector) LIKE ? OR
        (SELECT nome FROM users WHERE users.id = d.user) LIKE ? OR
        (SELECT responsavel FROM sector WHERE sector.id = d.sector) LIKE ?
";
...
$sql = "
    SELECT 
        d.id, CONCAT('RI#', d.serie, '/', d.n_doc) AS descricao, d.motivo, d.solicitante, 
