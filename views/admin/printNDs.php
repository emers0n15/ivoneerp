...
$titulo = 'RELATÓRIO DE NOTAS DE DÉBITO - ' . $data1 . ' / ' . $data2;
$pdf = new ThemedTCPDF('P', 'mm', 'A4', true, 'UTF-8', false, $dados, $titulo);
$pdf->SetCreator('IvoneERP');
$pdf->SetTitle('Relatório de Notas de Débito');
$pdf->AddPage();
...
$columns = [
    ['#', 12], ['Descrição', 35], ['Valor', 22], ['IVA', 18], ['Total', 25], ['Data de Emissão', 38], ['Data de Pagamento', 40]
];
tcpdf_table_header($pdf, $columns);

// Loop while para adicionar linhas de itens
$pdf->SetFont('helvetica', '', 8);
$sql3 = "SELECT DISTINCT(cliente) as cl FROM `nota_debito` as e WHERE data BETWEEN '$data1' AND '$data2'";
...
while ($dados3 = mysqli_fetch_array($rs3)) {
    ...
    $pdf->Ln(6);
    $pdf->SetFont('helvetica', '', 8);
    $sqlx = "SELECT * FROM nota_debito as f WHERE cliente = '$cl' AND f.data BETWEEN '$data1' AND '$data2' AND serie = '$serie' ORDER BY f.data";
    ...
    while ($dadosx = mysqli_fetch_array($rsx)) {
        ...
        $pdf->Cell(35, 6, "ND#".$dadosx['serie']."/".$dadosx['n_doc'], 1, 0, 'L', $fill);
        $pdf->Cell(22, 6, number_format($dadosx['valor'], 2), 1, 0, 'R', $fill);
        $pdf->Cell(18, 6, number_format($dadosx['iva'], 2), 1, 0, 'R', $fill);
        $pdf->Cell(25, 6, number_format($ta, 2), 1, 0, 'R', $fill);
        $pdf->Cell(38, 6, $dadosx['data'], 1, 0, 'C', $fill);
        $pdf->Cell(40, 6, $dadosx['prazo'], 1, 1, 'C', $fill);
...
// Output do PDF
$pdf->Output('Relatorio_notas_debito.pdf', 'I');
