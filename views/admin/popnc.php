...
$countSql = "SELECT COUNT(*) AS total FROM nota_de_credito";
...
$filterSql = "
    SELECT COUNT(*) AS filtered FROM nota_de_credito AS n
    WHERE
        n.id LIKE ? OR
        n.motivo LIKE ? OR
        CONCAT('NC#', n.serie, '/', n.n_doc) LIKE ? OR
        (SELECT CONCAT(nome,' ',apelido) FROM clientes WHERE clientes.id = n.cliente) LIKE ? OR
        (SELECT CONCAT('FA#', f.serie, '/', f.n_doc) FROM factura AS f WHERE f.id = n.id_factura) LIKE ? OR
        (SELECT nome FROM users WHERE users.id = n.user) LIKE ?
";
$stmtFiltered = $db->prepare($filterSql);
$stmtFiltered->bind_param('ssssss', $searchValue, $searchValue, $searchValue, $searchValue, $searchValue, $searchValue);
...
$sql = "
    SELECT 
        n.id, CONCAT('NC#', n.serie, '/', n.n_doc) AS descricao, n.valor, n.iva, n.serie, 
        (SELECT CONCAT('FA#', f.serie, '/', f.n_doc) FROM factura AS f WHERE f.id = n.id_factura) AS factura, 
        n.motivo, 
        (SELECT CONCAT(nome,' ',apelido) FROM clientes WHERE clientes.id = n.cliente) as clientex,
        (SELECT nome FROM users WHERE users.id = n.user) as usuario
    FROM 
        nota_de_credito AS n
    WHERE
        n.id LIKE ? OR
        n.motivo LIKE ? OR
        CONCAT('NC#', n.serie, '/', n.n_doc) LIKE ? OR
        (SELECT CONCAT(nome,' ',apelido) FROM clientes WHERE clientes.id = n.cliente) LIKE ? OR
        (SELECT nome FROM users WHERE users.id = n.user) LIKE ? OR
        (SELECT CONCAT('FA#', f.serie, '/', f.n_doc) FROM factura AS f WHERE f.id = n.id_factura) LIKE ?
    LIMIT $start, $length
";
